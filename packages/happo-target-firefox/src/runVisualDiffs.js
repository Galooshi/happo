...
function resolveViewports(example, options) {
  const viewports = example.options.viewports ||
    Object.keys(options.viewports).slice(0, 1);

  return viewports.map(viewport =>
    Object.assign({}, options.viewports[viewport], { name: viewport }));
}
      ...
      .then((examples) => {
        if (!examples.length) {
          ...
          examples.forEach((example) => {
            resolveViewports(example, options).forEach((viewport) => {
...
function takeCroppedScreenshot({ driver }) {
  return new Promise((resolve, reject) => {
    driver.findElement(By.id(Constants.SCREENSHOT_BOX_ID)).then((overlay) => {
